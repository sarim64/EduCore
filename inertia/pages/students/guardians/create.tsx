...
export default function CreateGuardianPage({ studentId }: { studentId?: string | null }) {
  ...
  return (
    <>
      ...
      <div className="max-w-3xl">
        <div className="mb-6">
          {studentId ? (
            <Link href={`/students/${studentId}`} className="text-blue-600 hover:underline">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex gap-2">
            <Button type="submit" disabled={processing}>
              {processing ? 'Creating...' : 'Create Guardian'}
            </Button>
            <Button asChild type="button" variant="outline">
              <Link href={studentId ? `/students/${studentId}` : '/guardians'}>Cancel</Link>
            </Button>
          </div>
        </form>
      </div>
...
CreateGuardianPage.layout = (page: ReactElement) => <DashboardLayout>{page}</DashboardLayout>
